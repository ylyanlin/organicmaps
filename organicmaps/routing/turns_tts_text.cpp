...
std::string GetYouArriveTextId(Notification const & notification)
{
  ...
  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_4][%ld] GetYouArriveTextId called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  if (!notification.IsPedestrianNotification() &&
...
std::string GetDirectionTextId(Notification const & notification)
{
  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_2][%ld] GetDirectionTextId called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  if (notification.IsPedestrianNotification())
  {
