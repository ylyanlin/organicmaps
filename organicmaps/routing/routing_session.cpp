...
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_9][%ld] GetRouteFollowingInfo called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  CHECK_THREAD_CHECKER(m_threadChecker, ());
...
double RoutingSession::GetCompletionPercent() const
{
  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[%ld] GetCompletionPercent called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  CHECK_THREAD_CHECKER(m_threadChecker, ());
  ASSERT(m_route, ());
                                          ...
                                          location::RouteMatchingInfo & routeMatchingInfo)
{
  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[%ld] MatchLocationToRoute called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  CHECK_THREAD_CHECKER(m_threadChecker, ());
  if (!IsOnRoute())
    return true;
