...
//    __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_13][%ld] TooCloseForFisrtNotification called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
...
double Settings::ConvertUnitsToMeters(double distanceInUnits) const
{
    timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
//    __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_10][%ld] ConvertUnitsToMeters called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  switch (m_lengthUnits)
  {
...
double Settings::ConvertMetersToUnits(double distanceInMeters) const
{
    timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
//    __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_11][%ld] ConvertMetersToUnits called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  switch (m_lengthUnits)
  {
...
uint32_t Settings::ComputeDistToPronounceDistM(double speedMetersPerSecond, bool pedestrian) const
{
    timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
//    __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_12][%ld] ComputeDistToPronounceDistM called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
