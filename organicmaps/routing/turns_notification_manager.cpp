    ...
    bool useThenInsteadOfDistance, TurnItem const & turn, measurement_utils::Units lengthUnits) const
{
    ...
    __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_3][%ld] GenerateTurnText called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  // Prevent "Keep left / right" notification from being generated
  if (turn.m_turn != CarDirection::None && turn.m_turn != CarDirection::TurnSlightLeft && turn.m_turn != CarDirection::TurnSlightRight)
  {
                                                        ...
                                                        double distanceToTurnMeters)
{
  ...
  //timespec ts;
  //clock_gettime(CLOCK_MONOTONIC, &ts);
  //__android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_4][%ld] GenerateFirstTurnSound called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
    ...
    if (!m_settings.TooCloseForFisrtNotification(distanceToTurnMeters))
    {
      ...
      if (distanceToTurnMeters < startPronounceDistMeters)
      {
        ...
        else
        {
          ...
          return std::string{};
          // Prevent turn ahead notification e.g. "In 200 meters, turn left"
          // This is because we only want the notifications at the actual turns
...
void NotificationManager::FastForwardFirstTurnNotification()
{
    timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
//    __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_19][%ld] FastForwardFirstTurnNotification called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  m_turnNotificationWithThen = false;
  if (m_nextTurnNotificationProgress == PronouncedNotification::Nothing)
    m_nextTurnNotificationProgress = PronouncedNotification::First;
}
...
CarDirection NotificationManager::GenerateSecondTurnNotification(std::vector<TurnItemDist> const & turns)
{
  ...
  //timespec ts;
  //clock_gettime(CLOCK_MONOTONIC, &ts);
  //__android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_4][%ld] GenerateSecondTurnNotification called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  if (turns.size() < 2)
  {
    m_secondTurnNotificationIndex = 0;
