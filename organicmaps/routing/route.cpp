...
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_18][%ld] GetCurrentDistanceFromBeginMeters called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  if (!IsValid())
    return 0.0;
...
//  timespec ts;
//  clock_gettime(CLOCK_MONOTONIC, &ts);
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_5][%ld] GetCurrentDistanceToEndMeters called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  if (!IsValid())
    return 0.0;
