...
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_21][%ld] OnLocationPositionChanged called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
...
bool SpeedCameraManager::ShouldPlayBeepSignal()
{
    timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
//    __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_22][%ld] ShouldPlayBeepSignal called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  CHECK_THREAD_CHECKER(m_threadChecker, ());
...
void SpeedCameraManager::FindCamerasOnRouteAndCache(double passedDistanceMeters)
{
  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_23][%ld] FindCamerasOnRouteAndCache called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  CHECK(!m_route.expired(), ());
