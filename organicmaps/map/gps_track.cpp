...
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[%ld] AddPoint called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  {
...
hours GpsTrack::GetDuration() const
{
  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[%ld] GetDuration called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  lock_guard<mutex> lg(m_dataGuard);
  return m_duration;
}
...
void GpsTrack::ScheduleTask()
{
  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[%ld] ScheduleTask called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  lock_guard<mutex> lg(m_threadGuard);
