...
Java_app_organicmaps_Framework_nativeFollowRoute(JNIEnv * env, jclass)
{
  ...
  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_1][%ld] nativeFollowRoute called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
...
Java_app_organicmaps_Framework_nativeGenerateNotifications(JNIEnv * env, jclass)
{
  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
  //sidechannel::logMethodCall("nativeGenerateNotifications");
  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_6][%ld] nativeGenerateNotifications called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
...
Java_app_organicmaps_Framework_nativeGetRouteFollowingInfo(JNIEnv * env, jclass)
{
  timespec ts;
  clock_gettime(CLOCK_MONOTONIC, &ts);
//  __android_log_print(ANDROID_LOG_INFO, "SIDECHANNEL_DEBUG", "[MethodId_6][%ld] nativeGetRouteFollowingInfo called", (long)(ts.tv_sec * 1000000) + (ts.tv_nsec / 1000));
  ::Framework * fr = frm();
  if (!fr->GetRoutingManager().IsRoutingActive())
    return nullptr;
